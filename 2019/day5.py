...
def computer(L, i = 0, words = False, noun = 0, verb = 0, I1 = None, I2 = None, soo = False, paint = False):
    """ Op codes:
    ...
    if paint == True:
        ...
        y = 0
        x = 0
        direct = 0 #0 is up, 1 is right, 2 is down, 3 is left
        count_out = 0
    ...
    while True:
        ...
        if op == 99:
            if paint == False:
                return I2, 99, i
            else: return len(w_history)+len(b_history)
        # if L[i]//100%10 == 0:
        #     s1 = L[L[i+1]]
        # else:
        #     s1 = L[i+1]
        ...
        if op == 1:
            ...
            if paint == True:
                ...
                if count_out % 2 == 1:
                    if out == 0:
                        if (x,y) not in b_history: b_history.append((x,y))
                        if (x,y) in w_history:
                            w_history.remove((x,y))
                    else:
                        if (x,y) not in w_history: w_history.append((x,y))
                        if (x,y) in b_history:
                            b_history.remove((x,y))
